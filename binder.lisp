...
(defun parse-bind-slave-log (lines)
  (let* (ratchet
         ...
         (soareg (cl-ppcre:create-scanner "^.*IN.*SOA"))
         ;; (soaend (cl-ppcre:create-scanner ".*\)"))
         (record (cl-ppcre:create-scanner "(A|INFO|TXT|HINFO|LOC|CNAME|^\$INCLUDE)"))
         (ismx   (cl-ppcre:create-scanner "(MX)"))
         (isns   (cl-ppcre:create-scanner "(NS)"))
         (origin-top (cl-ppcre:create-scanner "^\$ORIGIN\s+\."))
         (origin-mid (cl-ppcre:create-scanner "(^\$ORIGIN\s+\w+\.\w+)"))
         soa-list
         ns-list
         mx-list
         red-list)
    (declare (ignorable ratchet soareg record ismx isns soa-list ns-list mx-list red-list))
    (loop for line in lines
          for count upfrom 1
          do (cond
               ((cl-ppcre:scan origin-top line)
               ...
               ((cl-ppcre:scan soareg line)
                (let ((soa (cl-ppcre:scan-to-strings soareg line)))
                  (format t "~&[~05D] Type: ~S: ~A" count soa line)))
               ((cl-ppcre:scan record line)
                (let ((record (cl-ppcre:scan-to-strings record line)))
                  (format t "~&[~05D] Type: ~A: ~A" count record line)))
